...
(function() {
  var CheckError, ComputeNode, DIR_TO_CHAR, DamagedNode, Emulator, EventEmitter, IMAGE_HEIGHT, IMAGE_LENGTH, IMAGE_PRINT_CHARS, IMAGE_WIDTH, ImageNode, InputNode, LINE_MARKER, MAX_COLUMNS, MAX_INT, MAX_LINES, MAX_STACK_SIZE, MAX_STREAM_LENGTH, MIN_INT, MemoryNode, Node, OP_ANY_READ_ORDER, OutputNode, Table, colors, debug, fs, parserlib, speclib, statslib,
  ...
  IMAGE_PRINT_CHARS = ['█'.black, '█'.gray.dim, '█'.gray, '█'.white, '█'.red];
  ...
  Emulator = (function(superClass) {
    ...
    Emulator.prototype.toString = function() {
      var a, actual, actualImage, arr, expected, expectedImage, i, j, k, l, len, len1, len2, m, name, node, o, ref, ref1, ref2, ref3, ref4, ref5, ret, row, table;
      this.calculateStats();
      ret = '';
      table = new Table();
      ...
      for (l = 0, len1 = ref1.length; l < len1; l++) {
        node = ref1[l];
        if (!(node instanceof OutputNode && !(node instanceof ImageNode))) {
          continue;
        }
      ...
      for (m = 0, len2 = ref2.length; m < len2; m++) {
        ...
        expected = node.expected, actual = node.actual;
        expectedImage = actualImage = '';
        for (i = o = 0, ref3 = IMAGE_LENGTH; 0 <= ref3 ? o < ref3 : o > ref3; i = 0 <= ref3 ? ++o : --o) {
          expectedImage += (ref4 = IMAGE_PRINT_CHARS[expected[i]]) != null ? ref4 : ' ';
          actualImage += (ref5 = IMAGE_PRINT_CHARS[actual[i]]) != null ? ref5 : ' ';
          if ((i + 1) % IMAGE_WIDTH === 0 && i + 1 !== IMAGE_LENGTH) {
            expectedImage += '\n';
            actualImage += '\n';
          }
        }
      ...
      for (nodeIndex = k = 0, len = ref1.length; k < len; nodeIndex = ++k) {
        ...
        if (this.spec != null) {
          ...
          for (o = 0, len3 = ref5.length; o < len3; o++) {
            ...
            switch (type) {
              case 'STREAM_IMAGE':
                if (stream.length !== IMAGE_LENGTH) {
                  throw new Error("Image stream is wrong size, expected " + IMAGE_LENGTH + " (" + IMAGE_WIDTH + "x" + IMAGE_HEIGHT + "), got " + stream.length);
                }
                break;
              case 'STREAM_OUTPUT':
  ...
  ImageNode = (function(superClass) {
    extend(ImageNode, superClass);

    function ImageNode(expected1) {
      var _;
      this.expected = expected1;
      ImageNode.__super__.constructor.call(this, this.expected);
      this.actual = (function() {
        var k, ref, results;
        results = [];
        for (_ = k = 0, ref = this.expected.length; 0 <= ref ? k < ref : k > ref; _ = 0 <= ref ? ++k : --k) {
          results.push(0);
        }
        return results;
      }).call(this);
    ...
    ImageNode.prototype._setPtr = function() {
      if (this.x >= IMAGE_WIDTH || this.y >= IMAGE_HEIGHT) {
        return;
      }
      return this.ptr = this.x + this.y * IMAGE_WIDTH;
    };

    ImageNode.prototype._advancePtr = function() {
      this.ptr++;
      if (this.ptr % IMAGE_WIDTH === 0) {
        return this.ptr = null;
      }
    };

    ImageNode.prototype._isPassed = function() {
      var correct, i, k, len, ref;
      ref = this.expected;
      for (i = k = 0, len = ref.length; k < len; i = ++k) {
        correct = ref[i];
        if (this.actual[i] !== correct && this.actual[i] > 0) {
          return false;
        }
      }
      return true;
    ...
    ImageNode.prototype._isFinished = function() {
      var correct, i, k, len, ref;
      ref = this.expected;
      for (i = k = 0, len = ref.length; k < len; i = ++k) {
        correct = ref[i];
        if (this.actual[i] !== correct) {
          return false;
        }
      }
      return true;
    ...
    ImageNode.prototype.stepOne = function() {
      var value;
      if (this.neighbor_up == null) {
        return;
      }
      ...
      if (value != null) {
        ...
        this.passed = this._isPassed();
        this.finished = this._isFinished();
        if (this.ptr != null) {
          this._advancePtr();
        }
      }
