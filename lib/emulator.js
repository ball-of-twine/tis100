...
(function() {
  ...
  MemoryNode = (function(superClass) {
    ...
    MemoryNode.prototype._allowExtraRead = function() {
      var arg1, dir, k, len, myDir, neighbor, neighbor_instruction, op, ref;
      debug(this.name, 'has been read from UP or LEFT:', this.port_up == null);
      if (this.port_up === null) {
        return true;
      }
      ref = ['RIGHT', 'DOWN'];
      for (k = 0, len = ref.length; k < len; k++) {
        ...
        debug(this.name, 'found', dir, 'neighbor');
        debug(this.name, dir, 'neighbor\'s ptr is:', neighbor.iptr);
        neighbor_instruction = neighbor.instructions[neighbor.iptr];
        if (neighbor_instruction == null) {
          continue;
        }
        debug(this.name, 'found neighbor instruction');
        op = neighbor_instruction[0], arg1 = neighbor_instruction[1];
        debug(this.name, 'neighbor instruction op,arg1:', op, arg1);
        if (op !== 'JRO' && op !== 'ADD' && op !== 'SUB' && op !== 'MOV') {
          continue;
        }
        if (arg1 === 'LAST' && neighbor.last === myDir) {
          return true;
        }
        if (arg1 === myDir) {
          return true;
    ...
    MemoryNode.prototype.stepOne = function() {
      ...
      for (k = 0, len = OP_ANY_READ_ORDER.length; k < len; k++) {
        ...
        if (this.stack.length = MAX_STACK_SIZE && !this._allowExtraRead()) {
          break;
        }
